 ...
 * @author burningface
 */
class Generator_Item_Allorm extends Generator_Item_Abstract_Item {
    ...
    protected function init() {
        ...
        foreach ($tables as $table){
            $table = Generator_Db_Table::factory($table);
            $orm = Generator_File_Orm::factory($table);
            $this->add(
                $item = Generator_File::factory()
                    ->setFileName(strtolower($table->getName()))
                    ->setDirectory("application".DIRECTORY_SEPARATOR."classes".DIRECTORY_SEPARATOR."model")
                    ->addLine("class Model_".Generator_Util_Text::upperFirst($table->getName())." extends Model_".Generator_Util_Text::upperFirst($extend_orm["parent"])." {\n")
